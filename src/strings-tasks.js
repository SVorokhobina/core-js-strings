...
function isPalindrome(str) {
  ...
  const strModif = arr.filter((a) => letters.includes(a)).join('');
  const strModifRevert = arr
    .filter((a) => letters.includes(a))
    .reverse()
    .join('');
  return strModif === strModifRevert;
}
...
function invertCase(str) {
  ...
  const arr = str.split('');
  for (let i = 0; i < arr.length; i += 1) {
    if (lowerCase.includes(arr[i])) {
      arr[i] = arr[i].toUpperCase();
    } else if (upperCase.includes(arr[i])) {
...
function encodeToRot13(str) {
  const strCodeStart = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const strCodeResult = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  const arr = str.split('');
  let pos;
  for (let i = 0; i < arr.length; i += 1) {
    if (strCodeResult.includes(arr[i])) {
      pos = strCodeStart.indexOf(arr[i]);
      arr[i] = strCodeResult.charAt(pos);
    }
  }
  return arr.join('');
}
